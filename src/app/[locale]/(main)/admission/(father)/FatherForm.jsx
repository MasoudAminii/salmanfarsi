...
const FatherForm = () => {
  const [formData, setFormData] = useState(() => {
    if (typeof window !== "undefined") {
      const savedData = localStorage.getItem("fatherFormData");
      return savedData
        ? {
            ...JSON.parse(savedData),
            medicalInformation: {
              hasSpecificIllness: null,
              ...(JSON.parse(savedData).medicalInformation || {}),
            },
          }
        : {
            firstname: "",
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      localStorage.setItem("fatherFormData", JSON.stringify(formData));
    }, 500);
  ...
  const handleMedicalResponse = (value) => {
    setFormData((prev) => ({
      ...prev,
      medicalInformation: {
        ...prev.medicalInformation,
        hasSpecificIllness: value,
      },
    }));
  };
      ...
      <div className="title mb-4">
        <h2 className="text-3xl font-bold tracking-tight text-slate-900">
          <span className="bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent">
            Personal details of the student's father:
          </span>
        </h2>
        <p className="mt-2 text-lg text-slate-500">
          Basic details about the father
        </p>
      </div>
        ...
        <div className="space-y-4 rounded-lg bg-blue-50 p-6">
          <div className="space-y-2">
            <h3 className="text-lg font-semibold text-gray-900">
              Does your child have a specific illness?
            </h3>
            <p className="text-sm text-gray-600">
              If applicable, please provide the relevant medical documents in
              person.
            </p>
          </div>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={() => handleMedicalResponse(true)}
              className={`flex-1 rounded-xl px-4 py-3 transition-all ${
                formData.medicalInformation.hasSpecificIllness === true
            ...
            <button
              type="button"
              onClick={() => handleMedicalResponse(false)}
              className={`flex-1 rounded-xl px-4 py-3 transition-all ${
                formData.medicalInformation.hasSpecificIllness === false
                  ? "bg-red-600 text-white ring-2 ring-red-500"
                  : "bg-white text-gray-700 ring-1 ring-gray-200 hover:bg-red-50 hover:ring-red-200"
              }`}
            >
              <div className="flex items-center justify-center gap-2">
                <svg
